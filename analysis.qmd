...
document.addEventListener('DOMContentLoaded', function() {
  // Use setInterval to wait until HTMLWidgets actually renders the Plotly graphs
  var checkExist = setInterval(function() {
    if (typeof Plotly !== 'undefined') {
      var plots = document.querySelectorAll('.js-plotly-plot');
      
      // Sadece grafikler DOM'da var ise ve Plotly layout objesi yüklendiyse çalış
      if (plots.length > 0 && plots[0].layout) {
        ...
        plots.forEach(function(plot, index) {
          ...
          // Download butonunu oluştur (Saf SVG URL'si ile)
          ...
          btn.onclick = function(e) {
            e.preventDefault();
            try {
              // Dinamik Dosya İsmi: Grafiğin title'ını okur, yoksa varsayılan isim verir
              var chartTitle = "LetItHappen_Chart_" + (index + 1);
              if (plot.layout && plot.layout.title && plot.layout.title.text) {
                 var rawTitle = plot.layout.title.text.replace(/<[^>]*>?/gm, ''); // HTML etiketlerini sil
                 chartTitle = rawTitle.replace(/[^a-zA-Z0-9]/g, '_').replace(/_+/g, '_'); // Boşlukları alt çizgi yap
              }
          ...
          // Butonu grafiğin hemen üstüne ekle
